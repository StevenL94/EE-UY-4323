...
define float @acc_b(float %b_in, i1 zeroext %b_en) nounwind uwtable {
  ...
  call void @llvm.dbg.value(metadata !{i1 %b_en_read}, i64 0, metadata !16), !dbg !25 ; [debug line = 3:31] [debug variable = b_en]
  %b_in_read = call float @_ssdm_op_Read.ap_auto.float(float %b_in) nounwind ; [#uses=1 type=float]
  call void @llvm.dbg.value(metadata !{float %b_in_read}, i64 0, metadata !26), !dbg !27 ; [debug line = 3:19] [debug variable = b_in]
  call void @llvm.dbg.value(metadata !{float %b_in}, i64 0, metadata !26), !dbg !27 ; [debug line = 3:19] [debug variable = b_in]
  call void @llvm.dbg.value(metadata !{i1 %b_en}, i64 0, metadata !16), !dbg !25 ; [debug line = 3:31] [debug variable = b_en]
  %cnt_load = load float* @cnt, align 4, !dbg !28 ; [#uses=2 type=float] [debug line = 6:3]
  br i1 %b_en_read, label %._crit_edge, label %1, !dbg !31 ; [debug line = 5:2]

; <label>:1                                       ; preds = %0
  %cnt_assign = fadd float %cnt_load, %b_in_read, !dbg !28 ; [#uses=2 type=float] [debug line = 6:3]
  store float %cnt_assign, float* @cnt, align 4, !dbg !28 ; [debug line = 6:3]
  br label %._crit_edge, !dbg !32                 ; [debug line = 7:2]

._crit_edge:                                      ; preds = %1, %0
  %cnt_loc = phi float [ %cnt_load, %0 ], [ %cnt_assign, %1 ] ; [#uses=1 type=float]
  ret float %cnt_loc, !dbg !33                    ; [debug line = 8:2]
}
...
!17 = metadata !{i32 786478, i32 0, metadata !18, metadata !"acc_b", metadata !"acc_b", metadata !"", metadata !18, i32 3, metadata !19, i1 false, i1 true, i32 0, i32 0, null, i32 256, i1 false, float (float, i1)* @acc_b, null, null, metadata !23, i32 3} ; [ DW_TAG_subprogram ]
...
!30 = metadata !{i32 786443, metadata !17, i32 3, i32 38, metadata !18, i32 0} ; [ DW_TAG_lexical_block ]
