...
@cnt = internal unnamed_addr global float 0.000000e+00, align 4 ; [#uses=2 type=float*]
...
define float @acc_b(float %b_in, i1 zeroext %b_en) nounwind uwtable {
  call void (...)* @_ssdm_op_SpecBitsMap(float %b_in) nounwind, !map !12
  call void (...)* @_ssdm_op_SpecBitsMap(i1 %b_en) nounwind, !map !18
  call void (...)* @_ssdm_op_SpecBitsMap(float 0.000000e+00) nounwind, !map !22
  call void (...)* @_ssdm_op_SpecTopModule([6 x i8]* @acc_b.str) nounwind
  call void @llvm.dbg.value(metadata !{float %b_in}, i64 0, metadata !28), !dbg !29 ; [debug line = 3:19] [debug variable = b_in]
  call void @llvm.dbg.value(metadata !{i1 %b_en}, i64 0, metadata !30), !dbg !31 ; [debug line = 3:31] [debug variable = b_en]
  %cnt.load = load float* @cnt, align 4, !dbg !32 ; [#uses=2 type=float] [debug line = 6:3]
  br i1 %b_en, label %._crit_edge, label %1, !dbg !35 ; [debug line = 5:2]

; <label>:1                                       ; preds = %0
  %cnt.assign = fadd float %cnt.load, %b_in, !dbg !32 ; [#uses=2 type=float] [debug line = 6:3]
  store float %cnt.assign, float* @cnt, align 4, !dbg !32 ; [debug line = 6:3]
  br label %._crit_edge, !dbg !36                 ; [debug line = 7:2]

._crit_edge:                                      ; preds = %1, %0
  %cnt.loc = phi float [ %cnt.load, %0 ], [ %cnt.assign, %1 ] ; [#uses=1 type=float]
  ret float %cnt.loc, !dbg !37                    ; [debug line = 8:2]
}
...
!3 = metadata !{i32 786484, i32 0, metadata !4, metadata !"cnt", metadata !"cnt", metadata !"", metadata !5, i32 4, metadata !8, i32 1, i32 1, float* @cnt} ; [ DW_TAG_variable ]
!4 = metadata !{i32 786478, i32 0, metadata !5, metadata !"acc_b", metadata !"acc_b", metadata !"", metadata !5, i32 3, metadata !6, i1 false, i1 true, i32 0, i32 0, null, i32 256, i1 false, float (float, i1)* @acc_b, null, null, metadata !10, i32 3} ; [ DW_TAG_subprogram ]
...
!15 = metadata !{metadata !"b_in", metadata !16, metadata !"float", i32 0, i32 31}
...
!25 = metadata !{metadata !"return", metadata !26, metadata !"float", i32 0, i32 31}
...
!34 = metadata !{i32 786443, metadata !4, i32 3, i32 38, metadata !5, i32 0} ; [ DW_TAG_lexical_block ]
