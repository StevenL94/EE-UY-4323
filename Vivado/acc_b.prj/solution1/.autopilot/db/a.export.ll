...
define i32 @acc_b(i32 %b_in, i1 zeroext %b_en) nounwind uwtable {
  ...
  %b_in_read = call i32 @_ssdm_op_Read.ap_auto.i32(i32 %b_in) nounwind
  %cnt_load = load i32* @cnt, align 4
  br i1 %b_en_read, label %._crit_edge, label %1

; <label>:1                                       ; preds = %0
  %cnt_assign = add nsw i32 %cnt_load, %b_in_read
  store i32 %cnt_assign, i32* @cnt, align 4
  br label %._crit_edge

._crit_edge:                                      ; preds = %1, %0
  %cnt_loc = phi i32 [ %cnt_load, %0 ], [ %cnt_assign, %1 ]
  ret i32 %cnt_loc
}
